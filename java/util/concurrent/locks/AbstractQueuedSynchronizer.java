...
public abstract class AbstractQueuedSynchronizer
    ...
    protected final boolean compareAndSetState(int expect, int update) {
        ...
        //compareAndSwapInt该方法为本地方法，有四个参数，分别代表：对象、对象的地址、预期值、修改值
    ...
    public class ConditionObject implements Condition, java.io.Serializable {
        ...
        private Node addConditionWaiter() {
            ...
            //当前线程新建节点，状态CONDITION = -2
            Node node = new Node(Thread.currentThread(), Node.CONDITION);
            /**
             * 将该节点加入到条件队列中firstWaiter、lastWaiter
             */
            if (t == null)
                firstWaiter = node;
            else
        ...
        *调用Condition的await()方法会使当前线程进入等待状态，同时会加入到Condition等待队列同时释放锁。
         *当从await()方法返回时，当前线程一定是获取了Condition相关连的锁。
         * 此段代码的逻辑是：首先将当前线程新建一个节点同时加入到条件队列中，然后释放当前线程持有的同步状态。
